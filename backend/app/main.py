...
async def startup():
    ...
    # Auto-seed the airport briefings / escape-route docs baked into the
    # repo, if not already loaded. No manual command needed -- happens on
    # every boot, so it self-heals even without persistent storage.
    try:
        conn = get_conn()
