...
# is ephemeral -- fine to run this way, but uploaded PDFs, bulletins,
# subscriber lists, and marketplace posts reset on every redeploy. Airport
# briefings baked into the repo re-seed automatically on boot either way
# (see startup() below), so that part is unaffected by the choice.
