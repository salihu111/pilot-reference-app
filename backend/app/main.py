...
def _marketplace_chat_url(client_id: str, thread_id: int | None = None) -> str | None:
    if not WEBAPP_URL or not client_id:
        return None
    marketplace_base = os.getenv("MARKETPLACE_URL") or f"{WEBAPP_URL.rstrip('/')}/marketplace.html"
    if not marketplace_base.startswith("http"):
        marketplace_base = f"https://{marketplace_base}"
    params = {"telegram": "1", "client_id": client_id, "v": int(time.time())}
    if thread_id is not None:
        params["thread_id"] = str(thread_id)
    return f"{marketplace_base}?{urllib.parse.urlencode(params)}"


def _notify_client(client_id: str, text: str, *, thread_id: int | None = None):
    """Best-effort private Telegram notification for tg:<user_id> clients."""
    ...
    try:
        ...
        reply_markup = None
        chat_url = _marketplace_chat_url(client_id, thread_id)
        if chat_url:
            reply_markup = InlineKeyboardMarkup(
                [[InlineKeyboardButton("💬 Open chat", web_app=WebAppInfo(url=chat_url))]]
            )
        fut = _asyncio.run_coroutine_threadsafe(
            telegram_app.bot.send_message(chat_id=chat_id, text=text, reply_markup=reply_markup),
            telegram_loop,
        )
...
def get_marketplace_chat(listing_id: int, client_id: str):
    ...
    thread = get_or_create_thread_for_viewer(listing_id, client_id)
    if thread.get("_created"):
        _notify_client(
            thread["owner_client_id"],
            f"💬 New Crew Market chat\n{thread.get('listing_title') or 'Crew Market'}\nA crew member opened a chat about your post.",
            thread_id=thread["id"],
        )
    return {"is_owner": False, "thread": thread}
...
def get_marketplace_thread_with(listing_id: int, client_id: str, counterpart_client_id: str):
    try:
        thread = get_or_create_thread_with(
            listing_id,
            owner_client_id=client_id,
        ...
        if thread.get("_created"):
            _notify_client(
                thread["counterpart_client_id"],
                f"💬 New Crew Market chat\n{thread.get('listing_title') or 'Crew Market'}\nThe listing owner opened a chat with you.",
                thread_id=thread["id"],
            )
        return thread
...
def post_marketplace_message(thread_id: int, client_id: str = Form(...), text: str = Form(...)):
    try:
        result = add_message(thread_id, client_id, text)
        preview = str(result.get("text") or "").replace("\n", " " ).strip()[:140]
        _notify_client(
            result["recipient_client_id"],
            f"💬 New message on Crew Market\n{result.get('listing_title') or 'Crew Market'}\n\"{preview}\"",
            thread_id=thread_id,
        )
        return result
    except PermissionError as e:
