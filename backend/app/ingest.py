...
def get_embedder():
    """Thread-safe singleton. Without the lock, two background threads
    starting around the same time (airport seeding + reference-PDF seeding
    both fire on startup, see main.py) can each see _embedder as None and
    each construct their own TextEmbedding -- loading the ONNX model into
    memory twice at once, right when memory is already tightest. On a
    memory-constrained Railway plan that's enough to get the whole
    container OOM-killed mid-boot."""
    global _embedder
