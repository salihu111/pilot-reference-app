...
def init_db():
    ...
    c.executescript(
        ...
        -- Last known METAR/TAF + NOTAM pull per airport, so the UI has
        -- something to show instantly on open instead of a blank/loading
        -- state, and so we're not forced to hit AVWX/AWC on every page
        -- view. Refreshed by /api/airports/{icao}/sync (manual button or
        -- the frontend's auto-refresh timer while a card is open).
        CREATE TABLE IF NOT EXISTS airport_weather_cache (
